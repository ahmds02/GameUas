...
def reset_game():
    global enemies, fast_enemies, strong_enemies, enemy_hits, fast_enemy_hits, strong_enemy_hits, player_hits, player_lives, score
    # Inisialisasi ulang musuh
    enemies = create_enemies(enemy_img, 6)
    fast_enemies = create_enemies(enemy_fast_img, 3)
    strong_enemies = create_enemies(enemy_strong_img, 3)
    
    # Inisialisasi ulang hitungan hit
    enemy_hits = [0] * len(enemies)
    fast_enemy_hits = [0] * len(fast_enemies)
    strong_enemy_hits = [0] * len(strong_enemies)
    player_hits = 0
...
# Fungsi untuk menampilkan nyawa pemain
def display_lives():
    global player_lives
    font = pygame.font.Font(None, 36)
...
def main_menu():
    while True:
        ...
        font = pygame.font.Font(None, 48)
        high_score_text = font.render("Higt Score (H)", True, WHITE)
